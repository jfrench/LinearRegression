...
The information above is often presented in an **analysis of variation (ANOVA) table** which is convenient for summarizing and organizing the various ingredients in the $F$-statistic. Below is a typical ANOVA table when $q=1$.
...
If the response has no relationship with the regressor variables, then we should be able to randomly permute the response variable ($y$) without a substantial difference in the typical model results. **A permutation of a vector can be obtained in R using the `sample` function.**
...
We would like to test whether the variables Elevation and Area should be used as regressors for Species.
...
```{r}
## I include this here since I'd like to run permutation test below
data(gala, package = "faraway")
lmod <- lm(Species ~ Area + Nearest, data = gala)
lms <- summary(lmod)
lms$fstat
...
## Question 7: Construct a 95% confidence interval to estimate the value of $\beta_{\rm Elevation}$.
...
# theme_bw makes the plot nicer for printing Black/White
