...
$p$ regression coefficients such as the one defined in Equation \@ref(eq:model-def-inference). We refer to this model as the "Full Model" and use the abbreviation $FM$.

We want to compare $FM$ to a "Reduced Model" ($RM$) that is a special case of $FM$. The most common examples of $RM$ are:

1. All of the coefficients except the intercept are set to zero.
2. Some of the coefficients are set to zero.

Less commonly considered examples of RM set certain coefficients equal to each other or place other restrictions on the coefficients. We emphasized that $RM$ is a special case of $FM$. Alternatively, this relationship is sometimes described by saying $RM$ is *nested* in $FM$.

Let $RSS_{RM}$ denote the RSS of the $RM$ and $RSS_{FM}$ denote the RSS of the $FM$. Similarly, $df_{RM}$ and $df_FM$ denote the degrees of freedom associated with the RSS for $RM$ and $FM$, respectively (recall that this is simply $n$, the number of observations, minus the number of estimated regression coefficients in the model being considered.)

Without be specific or developing complex notation, we cannot be precise in stating the hypotheses we wish to test. A general statement of our hypotheses is:

$H_0: RM\text{ is an adequate model for describing the population}$
$H_a: FM\text{ is a more appropriate model for describing the population.}$
...
Test of all the regressors (test for a regression relationship)
