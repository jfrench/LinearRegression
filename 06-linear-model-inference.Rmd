...
## Parametric confidence interval for the mean response {#parametric-ci-mean-response}
...
## Parametric prediction interval for a new response {#pi-new-response}
...
## Parametric hypothesis test for a single regression coefficient
...
Recall that if we make the additional assumptions in Equation
...
\end{equation}
Let's say we wish to test whether, for this model, $\beta_j$ differs from some constant number $c$ (typically, $c=0$). Let $\boldsymbol{\beta}_{-j}=\boldsymbol{\beta}\\{\beta_j}$, i.e., $\boldsymbol{\beta}_{-j}$ is the vector of all regression coefficients except $\beta_j$. $\boldsymbol{\beta}_{-j}$ has $p-1$ elements. We can state the hypotheses we wish to test as 
\[
\begin{aligned}
...
(\#eq:betaj-H0Ha)
\]
The first part of the null hypothesis in Equation \@ref(eq:betaj-H0Ha) states that $\beta_j = c$ while the first part of the alternative hypothesis assumes that $\beta_j \neq c$. But what does the second half of the hypotheses mean? The vertical bar means "assuming or conditional on", similar to the notation you would see in conditional probabilities or distributions. What this means is that we are performing the hypothesis test while assuming that the other coefficients are some real number (possibly even zero). Why state this at all? Isn't it implicitly assumed? No. We could perform a hypothesis test for $\beta_0$ for many different models that have differing regressors. By writing out hypotheses in the style of Equation \@ref(eq:betaj-H0Ha), we are being very clear that we are performing a test for $\beta_0$ in the context of the model that has the regressors associated with $\boldsymbol{\beta}_{-j}$.
...
p = 2P(t_{n-p}\geq |T_j|).
$$
What if we wished to test a one-sided hypothesis? The p-value for a lower-tailed test is $p=P(t_{n-p}\leq T_j)$ while the p-value for an upper-tailed test is $p=P(t_{n-p}\geq T_j)$.
