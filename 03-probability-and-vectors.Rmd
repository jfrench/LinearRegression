...
A random variable $Y\sim \mathrm{Bernoulli}(\theta)$ when $\Omega = \{0, 1\}$ and $P(Y=1)=\theta$.
...
A random variable $Y\sim \mathrm{Bin}(n,\theta)$ is the sum of $n\geq 1$ (fixed) independent Bernoulli random variables. The associated sample space is $\Omega=\{0,1,2,…,n\}$ and the pmf is $$f(y│\theta) = {n \choose y} \theta^y (1-\theta)^{(n-y)}.$$
...
$Y \sim \mathrm{Poisson}(\theta)$ when $\Omega=\{0,1,2,…\}$ and $$f(y|\theta)=\frac{1}{y!} \theta^y e^{-\theta}.$$
