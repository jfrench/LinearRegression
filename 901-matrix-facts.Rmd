...
```{r deriv-scalar, echo = FALSE}
deriv_df = data.frame(
  f = c("$cy$", "$y^2$", "$c y^2$"),
  df = c("$c$", "$2y$", "$2cy$")
)
knitr::kable(deriv_df, col.names = c("$f(y)$", "$\\frac{df(y)}{dy}$"), align = c("c", "c"), caption = "Some basic calculus results for scalar functions taking scalar inputs.")
```
...
knitr::kable(deriv_df2, col.names = c("$f(\\mathbf{y})$", "$\\frac{df(\\mathbf{y})}{d\\mathbf{y}}$"), align = c("c", "c"), caption = "Some basic calculus results for scalar functions taking vector inputs.")
```
