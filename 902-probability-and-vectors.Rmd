...
knitr::kable(prob_tab1, caption = "Basic terminology used in probability.")
```
...
- The set of outcomes of $\Omega$ are referred to as the **elements** of $\Omega$.
...
- **Addition rule**: $P(A\cap B) = P(A) + P(B) - P(A \cap B)$.
...
\[ \mathbf{y}\sim \mathsf{N}(0_{n\times 1}, \sigma^2 \mathbf{I}_{n\times n}).
\]

The matrix $\mathbf{H}=\mathbf{X}(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T$ projects $\mathbf{y}$ into the space spanned by vectors in $\mathbf{X}$. Because of what we know about linear functions of a multivariate normal random vector (\@ref(linear-functions-of-a-multivariate-normal-random-vector)), we can determine that
\[
\mathbf{Hy}\sim \mathsf{N}(0_{n\times n},\sigma^2 \mathbf{H}).
\]
