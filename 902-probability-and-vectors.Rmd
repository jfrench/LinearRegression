...
Using the definitions and results in \@ref(random-vectors), we want to answer **Q7** of the example in \@ref(continuous-bivariate-distribution-example). Summarizing only the essential details, we have a random column vector $\mathbf{z}=[X, Y]$ with mean $E(\mathbf{z})=[2/5, 4/5]$ and covariance matrix
\[
\mathrm{var}(\mathbf{z})=
\begin{bmatrix}
14/225 & 1/75 \\
...
\]
be an $n$-dimensional column vector of responses. It is common to assume that 
\[
