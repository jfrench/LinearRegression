---
title: Chapter 5 - Basic theoretical results
...
<a href="https://colab.research.google.com/github/jfrench/LinearRegression/blob/master/notebooks/05-linear-model-theory-notebook.ipynb">
   <img src="https://raw.githubusercontent.com/jfrench/LinearRegression/3d1b80d7964e80507900220afb30957eb9d40457/images/qr-linear-model-theory.svg">
</a>
...
$$
Y_i=\beta_0+\beta_1 x_{i,1} +\ldots + \beta_{p-1}x_{i,p-1}+\epsilon_i,\quad i=1,2,\ldots,n,
$$
...
We now show that the OLS estimators of the simple linear regression coefficients are unbiased. We start here because no matrices are involved, which makes the proof for accessible.
...
$$
\begin{aligned}
E(Y_i\mid X=x_i) &= E(\beta_0 + \beta_1 x_i +\epsilon_i\mid X = x_i) & \tiny\text{(substitute definition of $Y_i$)} \\
&= E(\beta_0\mid X=x_i) + E(\beta_1 x_i \mid X = X_i) +E(\epsilon_i \mid X=x_i) & \tiny\text{(linearity property of expectation)} \\
&= \beta_0+\beta_1x_i +E(\epsilon_i \mid X=x_i) & \tiny\text{(the $\beta$s and $x_i$ are non-random values)} \\
...
$$
\begin{aligned}
E\left(\sum x_iY_i \Bigm\vert \mathbf{X} \right) &= \sum E(x_iY_i \mid \mathbf{X}) &\tiny\text{ (by the linearity of the expectation operator)}\\
&=\sum x_iE(Y_i\mid \mathbf{X})&\tiny(x_i\text{ is a fixed value, so it can be factored out of the expectation})\\
...
&= E\left(\frac{1}{n}\sum Y_i \Bigm\vert  \mathbf{X} \right)&\tiny\text{(definition of sample mean)}\\
&= \frac{1}{n}E\left(\sum Y_i \Bigm\vert  \mathbf{X}\right)&\tiny\text{(factor out constant)}\\
...
&= E\left(\frac{\sum x_iY_i -\frac{1}{n}\sum x_i\sum Y_i}{\sum x_i^2-\frac{1}{n}\left(\sum  x_i\right)^2} \Biggm\vert \mathbf{X} \right) &\tiny\text{(substitute OLS estimator)} \\
&= \frac{1}{SSX}E\left(\sum x_iY_i-\frac{1}{n}\sum x_i\sum Y_i \Bigm\vert \mathbf{X}\right)&\tiny\text{(factor out constant denominator, substitute }SSX\text{)} \\
&= \frac{1}{SSX}\left[E\left(\sum x_iY_i\Bigm\vert\mathbf{X}\right)-E\left(\frac{1}{n}\sum x_i\sum Y_i\Bigm\vert\mathbf{X}\right)\right]&\tiny\text{(linearity of expectation)}\\
&= \frac{1}{SSX}\left[E\left(\sum x_iY_i\Bigm\vert\mathbf{X}\right)-\left(\sum x_i\right)E\left(\bar Y\mid \mathbf{X}\right)\right]&\tiny\text{(factor out constant }\sum x_i\text{, use definition of}\bar{Y})\\
...
Applying the Standard Assumptions to our linear model, we have:
...
$$
\begin{aligned}
E(\mathbf{y}\mid\mathbf{X})&=E(\mathbf{X}\boldsymbol{\beta}+\boldsymbol\epsilon\mid\mathbf{X})&\tiny\text{(by definition)}\\
&=E(\mathbf{X}\boldsymbol{\beta}\mid\mathbf{X})+E(\boldsymbol\epsilon\mid\mathbf{X})&\tiny\text{(linearity of expectation)}\\
&=E(\mathbf{X}\boldsymbol{\beta} \mid \mathbf{X})+\mathbf{0}_{n\times 1}&\tiny\text{(by assumption about }\epsilon)\\
...
$$
\begin{aligned}
\text{var}(\mathbf{y}\mid\mathbf{X})&=\text{var}(\mathbf{X}\boldsymbol{\beta}+\boldsymbol\epsilon\mid\mathbf{X})&\tiny\text{(by definition)}\\
&=\text{var}(\boldsymbol\epsilon\mid\mathbf{X})&\tiny(\mathbf{X}\boldsymbol{\beta}\text{ is constant)}\\
...
Since $\mathbf{y}$ is a linear function of the multivariate normal vector $\boldsymbol{\epsilon}$, then $\mathbf{y}$ must also have a multivariate normal distribution. Therefore, $\mathbf{y}\mid \mathbf{X}\sim \mathsf{N}(\mathbf{X}\boldsymbol{\beta}, \sigma^2 \mathbf{I})$.
...
$$
\begin{aligned}
E(\hat{\boldsymbol{\beta}}\mid\mathbf{X})&=E((\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{y}\mid\mathbf{X})&\tiny\text{(substitute OLS formula)}\\
&=(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^TE(\mathbf{y}\mid\mathbf{X})&\tiny(\text{factor non-random terms)}\\
...
$$
\begin{aligned}
\text{var}(\hat{\boldsymbol\beta}\mid\mathbf{X})&=\text{var}((\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{y}\mid\mathbf{X})&\tiny\text{(by OLS formula)}\\
&=(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\text{var}(\mathbf{y}\mid\mathbf{X})((\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T)^T&\tiny\text{(pull constant terms out of variance)}\\
&=(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\text{var}(\mathbf{y}\mid\mathbf{X})\mathbf{X}(\mathbf{X}^T\mathbf{X})^{-1}&\tiny\text{(simplification)}\\
...
$$
\hat{\boldsymbol\beta}\mid\mathbf{X} \sim N(\boldsymbol\beta,\sigma^2(\mathbf{X}^T\mathbf{X})^{-1}).
$$
...
$$
\begin{aligned}
E(\hat{\boldsymbol{\epsilon}}\mid\mathbf{X})&=E((\mathbf{I}-\mathbf{H})\mathbf{y}\mid\mathbf{X})\\
&=(\mathbf{I}-\mathbf{H})E(\mathbf{y}\mid\mathbf{X})&\tiny(\mathbf{I}-\mathbf{H}\text{ is non-random)}\\
...
$$
\begin{aligned}
\text{var}(\hat{\boldsymbol{\epsilon}}\mid\mathbf{X})&=\text{var}((\mathbf{I}-\mathbf{H})\mathbf{y}\mid\mathbf{X})\\
&=(\mathbf{I}-\mathbf{H})\text{var}(\mathbf{y}\mid\mathbf{X})(\mathbf{I}-\mathbf{H})^T&\tiny(\mathbf{I}-\mathbf{H}\text{ is nonrandom, factor out of var)}\\
