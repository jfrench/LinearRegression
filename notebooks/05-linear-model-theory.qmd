...
**Error Assumption #1:** the mean of the errors is zero. This means that $E(\epsilon_i \mid \mathbb{X} = \mathbf{x}_i)=0$ for $i=1,2,\ldots,n$, or using matrix notation:
...
**Error Assumption #2:** the errors have constant variances and are uncorrelated, conditional on knowing the regressors:
...
**Error Assumption #3:** the errors are identically distributed. Formally, this may be written as:
...
**Error Assumption #4:** In practice, it is common to assume the errors have a normal (Gaussian) distribution. Two uncorrelated normal random variables are also independent (this is true for normal random variables, but is not generally true for other distributions). Thus:
...
1. $\hat{\boldsymbol{\beta}}\mid \mathbf{X}\sim \mathcal{N}(\boldsymbol{\beta}, \sigma^2(\mathbf{X}^T\mathbf{X})^{-1})$.
1. $\hat{\boldsymbol{\epsilon}}\mid \mathbf{X}\sim \mathcal{N}(\mathbf{0}_{n\times 1}, \sigma^2 (\mathbf{I}_{n\times n} - \mathbf{H}))$.
...
:::{.theorem #mean-y}
For our given linear model and under the assumptions summarized previously:
...
Our response variable has the following distribution: 

$$
\mathbf{y}\mid \mathbf{X}\sim \mathcal{N}(\mathbf{X}\boldsymbol{\beta}, \sigma^2 \mathbf{I})
$$
:::
...
Under the assumptions we discussed previously, the errors have mean:
...
The errors have the following distribution:

$$
\hat{\boldsymbol{\epsilon}}\mid \mathbf{X}\sim \mathcal{N}(\mathbf{0}_{n\times 1}, \sigma^2 (\mathbf{I} - \mathbf{H}))
$$
