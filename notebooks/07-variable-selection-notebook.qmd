...
<a href="https://colab.research.google.com/github/jfrench/LinearRegression/blob/master/notebooks/07-variable-selection-notebook.ipynb">
   <img src="https://raw.githubusercontent.com/jfrench/LinearRegression/ca710f0fdb00102805333e9407b5cc271da0d59f/images/qr-variable-selection.svg">
</a>
...
Similar to what has been suggested by @rabe2015 and @lmwr2, we will attempt to find the simplest model that adequately explains the data.
...
- An exhaustive search is not feasible unless the number of regressors is relatively small.
    ...
    - Approaches for reducing the computational cost of the exhaustive approach are available, e.g., @schatzoff1968 and @furnival1971.
...
The *best subset* search strategy determines the model $\mathcal{M}$ that minimizes the $RSS_{\mathcal{M}}$ for each value of $p_{\mathcal{M}}$, where $1\leq p_{\mathcal{M}} \leq p$ [@hastie_tibs_tibs_2020].
...
- @furnival1974 proposed a "leaps and bounds" algorithm to efficiently perform best subset selection.
...
@aic proposed the most famous information criterion, which is now known as the Akaike Information Criterion (AIC).
...
@bic proposed the Bayesian Information Criterion (BIC), which is very similar to the AIC statistic. 
...
@alsm2005 summarize how to use Mallow's $C_p$ statistic.
...
*Ridge regression* [@ridge] seeks to choose the estimated coefficients that minimizes the objective function 
...
The LASSO (least absolute shrinkage and selection operator) method proposed by @lasso seeks to minimize the objective function
...
The elastic net [@elasticnet] is a compromise between the ridge and LASSO regression methods and seeks to minimize 
...
Assume the categorical predictor $C$ has three levels, $L_1$, $L_2$, and $L_3$ and that we are fitting the regression model 
$$E(Y \mid X, C) = \beta_0 + \beta_1 X + \beta_2 D_2 + \beta_3 D_3,$$
where $D_2$ and $D_3$ are indicator variables for $L_2$ and $L_3$ respectively.
