...
We extract the estimated error standard deviation, $\sqrt{\hat{\sigma}^2}$, using the `sigma` function.
...
- We can add indicator variables $D_2, D_3, \ldots, L_K$ to a simple linear regression model to create a parallel lines model for each level of $C$.
...
Why didn't we add $D_1$ to the parallel lines model? Or $D_1$ and $D_1 X$ to the separate lines model?
