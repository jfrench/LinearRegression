...
In the present context, we want to model the height of adult daughters using their height of their mothers. The model we build is known as a **regression model**.
...
- We have observed variables $X_1, X_2, \ldots, X_{p-1}$ and $Y$ for each of $n$ subjects from some population.
- $x_{i,j}$ denotes the value of $X_j$ for observation $i$. 
- $Y_i$ denotes the value of $Y$ for observation $i$. 
- If there is only a single regressor, we can denote the single regressor as $X$ and the observed values of $X$ as $x_1, x_2, \ldots, x_n$. 
...
Consider these potential "best fitting" lines which are drawn on the scatter plot of the height data. Which one is best?
