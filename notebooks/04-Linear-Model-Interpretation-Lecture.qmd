---
title: Lecture 04 - Linear Model Interpretation
...
* A weight difference of 1 gram is negligible in the context of penguin
weights
* A bill length change of 0.004 mm is unlikely to be noticed

In the code below, we divide the `body_mass_g` variable by 1000 to convert the variable from grams to kilograms. We then fit the model regressing `bill_length_mm` on `body_mass_kg` and extract the
...
include the something like *after accounting for the other variables in
the model*.
...
as you add or remove regressors from a model?
...
- First, the `effects::predictorEffect` function to compute the information needed to draw the plot. 
- Then, the `plot` function to display the information.
...
-   `predictor`: the name of the predictor you want to plot. This is the
    "focal predictor".
-   `mod`: the fitted model. The function works with `lm` objects, but
...
which has an interaction between a categorical and numeric predictor. Fox et al. (2020) also discuss predictors in the *conditioning group*, which is the set of predictors that interact with the focal predictor.
...
discrete values. By default, to compute the
...
Chinstrap penguins tend to have the largest bill lengths for a given value of body mass and the bill lengths increase more quickly as a function of body mass then
...
This plot allows use to determine the effect of `species` on `bill_length_mm` when we vary `body_mass_g` over 5 discrete values. When varying `body_mass_g` across
...
It sometimes doesn't make sense to talk about the effect of adding a single indicator regressor when all of the other regressors are in the model. When we add a categorical predictor to our model, we simultaneously add $K-1$
indicator variables as regressors; we do not add the indicator variables
one-at-a-time. 

In general, we refer to regressors with this behavior as
...
`body_mass_g` alone, which we already considered. Examining the leverage plot for the interaction term `body_mass_g:species` , we see the points have only a weak linear relationship. Thus, we expect limited utility in adding the
...
denote the $n\times 1$ column vector of observed values for regressor $X_j$. (We can't use the
