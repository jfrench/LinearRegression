...
<img src="https://raw.githubusercontent.com/jfrench/LinearRegression/477d83c5c1088c740945fc4a3f84424461b3d7a7/images/qr-random-vectors.svg">
</a>
...
The variance of a random vector isn't a number. Instead, it is the matrix of covariances of all pairs of random variables in the random vector.
...
Alternatively, the variance matrix of $\mathbf{y}$ is called the **covariance matrix** or **variance-covariance matrix** of $\mathbf{y}$.

The variance matrix of $\mathbf{y}$ can also be computed as $$
\mathrm{var}(\mathbf{y}) = E(\mathbf{y}\mathbf{y}^T)-E(\mathbf{y})E(\mathbf{y})^T.
$$

Also, noting that $\mathrm{cov}(Y, Y) = \mathrm{var}(Y)$ for a random variable $Y$, we can write the covariance matrix of $\mathbf{y}$ as
$$
\begin{aligned}
