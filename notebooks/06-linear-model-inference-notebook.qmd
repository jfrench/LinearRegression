...
How would we perform a hypothesis test for exactly one of these regression coefficients?
...
Tests are model specific, so it is important to be clear exactly what model is under consideration.
...
- The third column (`Std. Error`) provides the estimated standard error ($\hat{\mathrm{se}}(\hat{\beta}_j)$) for each coefficient.
...
We want to compare the Complete Model to a "Reduced Model" that is a special case of the Complete Model.
...
Less commonly considered examples of a Reduced Model set certain coefficients equal to each other or place other restrictions on the coefficients.

We emphasize that the Reduced Model is a special case of the Complete Model. 

Alternatively, this relationship is sometimes described by saying the Reduced Model is *nested* in the Complete Model.
...
- $RSS(RM)$ denote the RSS of the Reduced Model.
- $RSS(CM)$ denote the RSS of the Complete Model.
- $df(RM)$ and $df(CM)$ denote the degrees of freedom associated with the RSS for the Reduced and Complete Models, respectively.
...
We want to perform a hypothesis test involving multiple regression coefficients in our model. 

A general statement of the hypotheses we will test are:
...
The Reduced Model in $H_0$ is a special case of the Complete Model with most of the regression coefficients set equal to zero.
