...
- e.g., bar plots, boxplots, histograms, density plots, dot plots (bad), pie charts (bad), etc.
...
A histogram:
- displays the distribution of a `numeric` variable. - counts the number of values falling into (usually) equal-sized "bins".
- are used to assess skewness, modality (the number of clear "peaks" in the plot), and to some extent, outliers.
...
- Is the variable unimodal or bimodal?
...
A parallel box plot is used to display the distribution of a `numeric` variable whose values are grouped based on each `level` of a `factor` variable. 
Parallel box plot are useful for determining if the distribution of a `numeric` variable substantially changes based on whether an observation has a certain `level` of a `factor`.
...
A multivariate plot displays relationships between 2 or more variables (so bivariate plots are technically multivariate plots). 

Multivariate plots are better created using **`ggplot2`** than **`base`**.
...
We use colorblind-friendly palette and some additional customizations below.
...
The **`plotly`** package [@R-plotly] is an R package to provide the capabilities of plotly <https://plotly.com/>, a well-known tool for creating interactive scientific plots. The `ggplotly` function will instantly make a `ggplot` interactive (though you may need to customize it for your needs). We provide two examples below.
