...
The interpretation of the intercept is often nonsensical! What if $X_1$ was heart rate?
...
It is not possible to increase $X$ while keeping $X^2$ fixed. 
...
`penguins` data in Section in Section \@ref(s:penguins-mlr2). The fitted
...
`body_mass_g`. Figure \@ref(fig:effect-plot-species-lmods) allows use to
...
Note that the `crossprod` function computes the crossproduct of two
...
crossproduct between `ones` and the two regressors. Since the
crossproducts are not zero, `X1` and `X2` are not orthogonal to the
column of ones.
...
the matrix of crossproducts for the columns of 1s, `x1`, `x2`, `x3`, and
`x4`.
...
crossproduct matrix above. The block containing `ones` and `X1` is
