...
### Penguins examples {#penguins-slr-mlr-interpretations}

To apply the interpretations given above, we interpret the models fit to the `penguins` data in Sections \@ref(s:penguins-slr) and \@ref(s:penguins-mlr).
...
Did you notice that the estimated coefficients for the intercept and $\mathtt{body\_mass\_g}$ changed between the fitted simple linear model and the fitted multiple linear regression model in the Penguins example in Section \@ref(penguins-slr-mlr-interpretations)? What is happening?
...
To summarize the interpretation of the coefficients in separate lines models like Equation \@ref(eq:ll-def-interp), assuming categorical predictor $C$ has $K$ levels instead of 3:
