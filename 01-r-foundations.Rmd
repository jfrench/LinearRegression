...
In R Studio, you can have multiple script files open. But when you execute code, the commands are sent to the same console. This can lead to mistakes where you refer to an object that was defined in another script, and the script will lead to an error if run by itself. After developing code, it is good practice to  clear your environment using the broom icon in the Environment pane and rerun everything to make sure it behaves as you expected.
...
* If you know the command for which you want help, then run `?command` (where command is replaced the name of the relevant command) in the Console, to access the documentation for the object.
    ...
    * When reading help pages, the **Arguments** section describes function inputs and the **Value** section describes outputs
    * It is helpful to scroll down the the **Examples** section, where you should be able to directly copy and paste a reproducible example into your R session.
