...
## Vectors: creation, assignment, and factors
...
The column vectors of a data frame may be accessed using `$` and specifying the name of the desired vector. 

Part of a data frame can also be access by thinking of at as a general matrix and specifying the desired rows or columns in square brackets after the object name. For example, if we had a data frame named `df`:
...
Execute the following commands in the Console:
```{r, eval=FALSE}
df3 <- data.frame(numbers = 1:5, characters = letters[1:5], logicals = c(TRUE, TRUE, FALSE, TRUE, FALSE))
