...
# Loading Data For This Exploration
...
$$\mbox{var}(\mathbf{x}_0^T \widehat{\boldsymbol\beta}) = ??$$
...
Let's consider a charming individual, Median Man. Median Man has median values for all of their characteristics. 
...
(male.ave <- apply(teengamb[teengamb$sex == ??, ], ??, ??))
(female.ave <- apply(teengamb[teengamb$sex == ??, ], ??, ??))
...
```{r}
# find average of predictor values
male.ave <- colMeans(teengamb[teengamb$sex == ??, ])
female.ave <- colMeans(teengamb[teengamb$sex == ??, ])
