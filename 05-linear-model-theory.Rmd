...
\[
\begin{align}
\text{var}(\mathbf{y}|\mathbf{X})&=\text{var}(\mathbf{X}\boldsymbol{\beta}+\boldsymbol\epsilon|\mathbf{X})&\tiny\text{(bydefinition)}\\
&=\text{var}(\boldsymbol\epsilon|\mathbf{X})&\tiny(\mathbf{X}\boldsymbol{\beta}\text{ is constant)}\\
&=\sigma^2\mathbf{I}_{n\times n}&\tiny\text{(by assumption)}\\
&\square
\end{align}
\]
:::
...
\[\text{var}(\mathbf{y}|\mathbf{X})=\sigma^2\mathbf{I}_{n\times n}\]
...
\end{align}
\]
Thus, $\mathbf{I}_{n\times n}-\mathbf{H}$ is symmetric.
...
\end{align}
\]
Thus, $\mathbf{I}_{n\times n}-\mathbf{H}$ is idempotent.
...
Since $\hat{\boldsymbol{\epsilon}}$  is a linear combination of multivariate normal vectors, and using previous results, it has mean $\mathbf{0}_{n\times1}$ and variance-covariance matrix $\sigma^2(\mathbf{I}-\mathbf{H})$.
